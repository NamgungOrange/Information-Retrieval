...
class BM:
	...
	def calc_sent_lm(self, sentence,doc_fdoc_len,fcol_len,term_col_freq):
		
		doc_len = doc_fdoc_len
		
		col_len = fcol_len	
		
		fterm_col_freq = term_col_freq
		self.conn = sqlite3.connect("/home/cs13422/tfidf_full.db")
		...
		for term in query:
			...
			term_doc_freq = cur.fetchall()[0][1]
				...
				if doc_id in score_lst:				
					score_lst[doc_id] *= pow(10,1+(float(term_freq)/doc_len[doc_id]/(fterm_col_freq[term])/float(col_len)))
					#score_lst[doc_id] *= 1+(float(term_freq)/doc_len[doc_id])/2 + (fterm_col_freq[term])/float(col_len))/2					
				else:				
					score_lst[doc_id] = pow(10,1+(float(term_freq)/doc_len[doc_id]/(fterm_col_freq[term])/float(col_len)))
					#score_lst[doc_id] = 1+(float(term_freq)/doc_len[doc_id])/2 + (fterm_col_freq[term])/float(col_len))/2 
		...
		self.conn.close();
		return score_lst	
	def get_col_freq(self):
		q_col_freq=0
		sterm = ""
		ffterm_col_freq = dict()
		with open("/home/cs13435/ir/report/doc/AP88_posting_list.txt") as f:
			for line in f:
				w2d = line.replace("\n", "").strip().split(":")
				sterm, docs = w2d[0].strip().split(), w2d[1].strip().split()
				col_freq = int(sterm[1].replace("[", "").replace("]",""))				
				ffterm_col_freq[sterm[0]] = col_freq				
		return ffterm_col_freq
	...
	def save_sorted_bm(self, save_path, query_file):
		...
		doc_fdoc_len, fcol_len = self.doc_len_col_len()
		term_col_freq = self.get_col_freq()
		#print (term_col_freq)
		...
		for query in querys:
			...
			sc_lst = self.calc_sent_lm(temp_query,doc_fdoc_len,fcol_len,term_col_freq)
...
#	print "DB_build!!"
	...
	scorer.save_sorted_bm("LM_result.txt", "topics.202-250.txt")
	print "Complete!!"
